...
void decide_power_mode()
{
  ...
  // Power mode selected based on dominant frequency
  if (f_k >= 0.5)
  {
  ...
  } else if (f_k >= 0.1 && f_k < 0.5 && idleCount < 5)
  {
    ...
    ++idleCount;
  } else if (f_k < 0.1 || idleCount == 5) // If it stays in idle mode for 5 cycles it enters power down mode.
  {
    Serial.print("POWER_DOWN MODE");
    idleCount = 0;
  }

  // Calculate delayTime to be twice the dominant frequency corresponding to Nyquist's sampling theorem
  if (f_k > 0)
  {
    delayTime = 1000 / (2 * f_k);
